...
// Validate that user has username, email, password.
func userInputValid(user models.User) error {

	if len(user.Email) < 1 || !isEmail(user.Email) {
...
func userExists(id string, p string) bool {
	db := database.DB
...
func CreateUser(c *fiber.Ctx) error {
	...
	if err := db.Create(&user).Error; err != nil {
		return c.Status(400).JSON(fiber.Map{"status": "error", "message": "Couldn't create user", "data": err})
	}
